...
## Independent baselines and fresh OAuth (2026-09-20)

The earlier "direct native" test bypassed the Go gateway but still used `runNative()` and its `onPayload` override. Two independent runners now remove that ambiguity:

- `PI_AUTH_FILE=/private/auth.json node runtime/pi/verify-sdk-direct.mjs` imports the pinned official Pi adapter directly, supplies ordinary SDK message context, and does not import `native.mjs`, override payloads, or install the observed WebSocket class. Its read-only fetch observer records raw response declarations without changing request options or bytes.
- `python3 tools/pi-integration/verify-codex-direct.py` runs the installed official Codex CLI against `https://chatgpt.com/backend-api/codex` using existing OAuth auth. To expose response-model evidence in native SSE traces, it uses a named custom provider pointing directly to that official URL, with WebSocket disabled. There is no relay, Sub2API, or Pi runtime. User configuration is ignored, no auth/config file is modified, and raw traces are consumed in memory rather than printed. An earlier built-in-provider WS run completed successfully but exposed no raw response model; it is not model-acceptance evidence.

The official CLI version was `0.155.0-alpha.9.2`; Pi was `0.85.1`. Both independent SSE baselines completed the exact-marker request, requested `gpt-6-astra`, and observed `gpt-5.6-luna` in raw response events. Both runners correctly failed model acceptance. These calls ran on the same Mac with the local OAuth auth; their final network egress was not independently measured.

Fresh browser authorization through the deployed native-Pi admin endpoints also completed. The browser account matched the local-auth email, and the exchanged token's ChatGPT account ID matched the existing token's account. The runtime obtained new access/refresh credentials. The browser's localhost callback was unreachable because the callback listener is inside the private container; the documented manual callback exchange through the admin endpoint completed successfully. No separate workspace chooser appeared, so intended-workspace entitlement is not independently established merely by this account-ID equality.

Using those new credentials, the unwrapped Pi baseline again completed but declared Luna. One real refresh through the deployed private runtime preserved owner/account/harness binding, and the refreshed access token completed another direct SDK request with the same Luna declaration. This proves one fresh authorization/refresh cycle, not long-duration refresh stability. The user's existing Codex auth file was not replaced; temporary credentials were used only for acceptance.

### Session-affinity correction and deployment

`nativePiSession` now selects `session-id`, then `session_id`, then a trimmed `prompt_cache_key`. Per-request `x-client-request-id` is no longer accepted as a stable continuation key. Requests containing only a tracing ID now receive the existing stable-session-required error instead of silently creating a different Pi session each turn.

The Go `TestNativePi*` suite and all 22 JavaScript tests passed. Gateway build `0.2.6-pi.3`, image `local/sub2api:pi-5ccd7e18d`, is deployed and healthy. Runtime image remains `local/sub2api-pi-runtime:takeover-20260920`. A fresh temporary account/group/key used the refreshed token and sent two real requests with no Session Header, the same cache key, and different tracing IDs. Both completed; runtime WS counters were `connectionsCreated=1`, then `connectionsReused=1`, with no SSE fallback. These two requests intentionally sent full context (`deltaRequests=0`), so this verifies affinity/connection reuse, not a new delta-continuation claim. Both responses still declared Luna. Temporary entities were removed.

The pre-update Compose override is saved at `~/docker/sub2api/backups/pi-affinity-20260920/docker-compose.override.yml.before`; restore it and recreate only `sub2api` to roll back the gateway. The prior image remains available.

The mismatch is now reproduced without our gateway or Pi payload wrapper and with fresh/refreshed credentials. The provider's underlying cause remains undetermined; these observations do not establish an IP restriction, hidden model identity, or workspace entitlement. Production mismatch rejection/notification and preservation of upstream auth/rate-limit error classes remain separate product work; neither baseline runner changes production enforcement.
