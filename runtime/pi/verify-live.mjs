...
let passed=true,modelPassed=true,transportPassed=true,outboundPassed=true;
async function run(turn,request){
 ...
 const acceptance=liveAcceptance(result,{model,transport,turn,expectedModels:process.env.SUB2API_EXPECT_MODELS});
 passed&&=acceptance.passed;modelPassed&&=acceptance.modelAcceptance.passed;
 transportPassed&&=acceptance.transportPassed;outboundPassed&&=acceptance.outboundPassed;
 console.log(JSON.stringify({turn,requestedModel:model,httpStatus:status||null,transport:result.transport,observation:result.evidence,outbound:result.outbound,continuation:result.continuation,...acceptance}));
 if(!acceptance.protocolPassed)throw Error('native_protocol_failed');
...
try{
 ...
 console.log(JSON.stringify({result:passed?'PASS':'FAIL',toolContinuationPassed:true,modelPassed,transportPassed,outboundPassed}));
