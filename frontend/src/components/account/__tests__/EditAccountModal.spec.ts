...
describe('EditAccountModal OpenAI 自动使用重置卡', () => {
  ...
  it('仅对 OpenAI OAuth 母账号显示，默认关闭且每周阈值为 100', () => {
    ...
    expect(parent.get('[data-testid="auto-reset-credit-7d-threshold"]').attributes('disabled')).toBeDefined()
    parent.unmount()
  ...
  it('保存每周阈值，并禁止把运行态回写到管理请求', async () => {
  ...
  it('开启后拒绝超出 0.1–100 范围的每周阈值', async () => {
